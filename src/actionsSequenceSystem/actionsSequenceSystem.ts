    ...
    export class SequenceBuilder {
        ...
        if(condition: () => boolean): SequenceBuilder {
            ...
            if (seq instanceof IfSequenceNode) {
                ...
                this.currentSequenceNode = (this.currentSequenceNode as SequenceNode).then(elseSeq)
            }
            else {
        ...
        breakWhile(): SequenceBuilder {
            if (this.whileNodeStack.length > 0) {
                this.currentSequenceNode = (this.currentSequenceNode as SequenceNode).then(new BreakWhileSequenceNode(this.whileNodeStack[this.whileNodeStack.length - 1]))
            }
            else {
